...
function createListeningSet() {
  ...
  while (exercises.length < 200) {
    ...
    loopIdx++;
    const uniqueText = s.text + ' (Frase #' + (exercises.length + 1) + ')';
...
function writeData(filename, exportName, data) {
...
console.log('Writing ser-estar...'); writeData('ser-estar.ts', 'SER_ESTAR_EXERCISES', createSerEstar());
console.log('Writing present-regular...'); writeData('present-regular.ts', 'PRESENT_REGULAR_EXERCISES', createConjugationSet('pr', presentRegularVerbs, 'Present tense', '-ar: -o, -as, -a, -amos, -áis, -an; -er: -o, -es, -e, -emos, -éis, -en; -ir: -o, -es, -e, -imos, -ís, -en.'));
console.log('Writing present-irregular...'); writeData('present-irregular.ts', 'PRESENT_IRREGULAR_EXERCISES', createConjugationSet('ir', presentIrregularVerbs, 'Present tense irregular', 'Common irregular verbs need to be memorized.'));
console.log('Writing preterite...'); writeData('preterite.ts', 'PRETERITE_EXERCISES', createConjugationSet('pt', preteriteVerbs, 'Preterite tense', '-ar: -é, -aste, -ó, -amos, -asteis, -aron; -er/-ir: -í, -iste, -ió, -imos, -isteis, -ieron.'));
console.log('Writing imperfect...'); writeData('imperfect.ts', 'IMPERFECT_EXERCISES', createConjugationSet('im', imperfectVerbs, 'Imperfect tense', '-ar: -aba, -abas, -aba, -ábamos, -abais, -aban; -er/-ir: -ía, -ías, -ía, -íamos, -íais, -ían.'));
console.log('Writing future...'); writeData('future.ts', 'FUTURE_EXERCISES', createConjugationSet('fu', futureVerbs, 'Future tense', 'Add endings (-é, -ás, -á, -emos, -éis, -án) to the infinitive.'));
console.log('Writing conditional...'); writeData('conditional.ts', 'CONDITIONAL_EXERCISES', createConjugationSet('co', conditionalVerbs, 'Conditional tense', 'Add endings (-ía, -ías, -ía, -íamos, -íais, -ían) to the infinitive.'));
console.log('Writing subjunctive...'); writeData('subjunctive.ts', 'SUBJUNCTIVE_EXERCISES', createConjugationSet('su', subjunctiveVerbs, 'Present Subjunctive', "Drop 'o' of yo form and add opposite endings (-e for -ar, -a for -er/-ir)."));
console.log('Writing reflexive...'); writeData('reflexive.ts', 'REFLEXIVE_EXERCISES', createReflexiveSet());
console.log('Writing object-pronouns...'); writeData('object-pronouns.ts', 'OBJECT_PRONOUNS_EXERCISES', createObjectPronounsSet());
console.log('Writing listening-tenses...'); writeData('listening-tenses.ts', 'LISTENING_TENSES_EXERCISES', createListeningSet());
