...
// Generate all 11 topics with 200 questions each (total 2200 questions)
...
// Data definitions for the remaining 10 topics
...
function createReflexiveSet() {
  ...
  const verbs = [
    ...
    { infinitive: "vestirse", forms: ["me visto", "te vistes", "se viste", "nos vestimos", "os vestís", "se me visten"] },
...
function createObjectPronounsSet() {
  ...
  while (exercises.length < 200) {
    ...
    const isShort = exercises.length % 3 === 2; // mix of mcq and short
...
function createListeningSet() {
  ...
  while (exercises.length < 200) {
    ...
    const prompt = `Listen to the sentence. Which tense is being used? (#${exercises.length + 1})`;
...
// practice exercises (2,200 exercises total). Organized by grammar topic
