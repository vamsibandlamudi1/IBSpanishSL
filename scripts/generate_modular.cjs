...
function writeDataset(filename, exportName, generatorFn) {
  ...
  console.log('Done ' + filename + ' (' + data.length + ' exercises)');
}
...
writeDataset('ser-estar.ts', 'SER_ESTAR_EXERCISES', createSerEstar);
writeDataset('present-regular.ts', 'PRESENT_REGULAR_EXERCISES', () => createConjugationSet('pr', presentRegularVerbs, 'Present tense', '-ar: -o, -as, -a, -amos, -áis, -an; -er: -o, -es, -e, -emos, -éis, -en; -ir: -o, -es, -e, -imos, -ís, -en.'));
writeDataset('present-irregular.ts', 'PRESENT_IRREGULAR_EXERCISES', () => createConjugationSet('ir', presentIrregularVerbs, 'Present tense irregular', 'Common irregular verbs need to be memorized.'));
writeDataset('preterite.ts', 'PRETERITE_EXERCISES', () => createConjugationSet('pt', preteriteVerbs, 'Preterite tense', '-ar: -é, -aste, -ó, -amos, -asteis, -aron; -er/-ir: -í, -iste, -ió, -imos, -isteis, -ieron.'));
writeDataset('imperfect.ts', 'IMPERFECT_EXERCISES', () => createConjugationSet('im', imperfectVerbs, 'Imperfect tense', '-ar: -aba, -abas, -aba, -ábamos, -abais, -aban; -er/-ir: -ía, -ías, -ía, -íamos, -íais, -ían.'));
writeDataset('future.ts', 'FUTURE_EXERCISES', () => createConjugationSet('fu', futureVerbs, 'Future tense', 'Add endings (-é, -ás, -á, -emos, -éis, -án) to the infinitive.'));
writeDataset('conditional.ts', 'CONDITIONAL_EXERCISES', () => createConjugationSet('co', conditionalVerbs, 'Conditional tense', 'Add endings (-ía, -ías, -ía, -íamos, -íais, -ían) to the infinitive.'));
writeDataset('subjunctive.ts', 'SUBJUNCTIVE_EXERCISES', () => createConjugationSet('su', subjunctiveVerbs, 'Present Subjunctive', "Drop 'o' of yo form and add opposite endings (-e for -ar, -a for -er/-ir)."));
writeDataset('reflexive.ts', 'REFLEXIVE_EXERCISES', createReflexiveSet);
writeDataset('object-pronouns.ts', 'OBJECT_PRONOUNS_EXERCISES', createObjectPronounsSet);
writeDataset('listening-tenses.ts', 'LISTENING_TENSES_EXERCISES', createListeningSet);

console.log('ALL_FILES_GENERATED_SUCCESSFULLY');
